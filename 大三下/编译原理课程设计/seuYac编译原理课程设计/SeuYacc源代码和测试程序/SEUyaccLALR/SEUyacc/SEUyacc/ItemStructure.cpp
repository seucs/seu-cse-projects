...
Symbol::Symbol(const Symbol & s):_name(s._name),_type(s._type)
{
}
...
ostream & operator<<(ostream & out, CalItemSets r)
{
	out << "itemSet \n";
...
bool itemSet::deepeq(const itemSet & s2)
{
	if (items.size() != s2.items.size())
		return false;
	for (auto it1 = items.begin(), it2 = s2.items.begin(); it1 != items.end(); it1++, it2++)
	{
		if (it1->dot != it2->dot || it1->ruleId != it2->ruleId || it1->predict.size() != it2->predict.size())
			return false;
		for (auto itt1 = it1->predict.begin(), itt2 = it2->predict.begin(); itt1 != it1->predict.end(); itt1++, itt2++)
		{
			if (!(*itt1 == *itt2))
				return false;
		}
...
void CalItemSets::setStartRule(int ruleId)
{
	set<Symbol> v;
	v.insert(Symbol("$", 1));
	start.addItem(item(ruleId,0,v));
}
void CalItemSets::genPDALR()
{
	start.closure();
	...
	while (!q.empty())
	{
		...
		for (Symbol s : cand)
		{
			...
			else
			{
				...
				edgeSet[pair<int, Symbol>(itemHash[temp.getString()], s)] = itemHash[vItemSet[ind].getString()];			
			}
		}
	}
}

void CalItemSets::genPDALALR()
{
	start.closure();
	vItemSet.push_back(start);
	itemHash[start.getStringLALR()] = index++;
	queue<itemSet> q;
	q.push(start);
	while (!q.empty())
	{
		...
		{
			itemSet p=temp.shift(s);
			if (itemHash[p.getStringLALR()] <= 0)
			{
				vItemSet.push_back(p);
				itemHash[p.getStringLALR()] = index++;			
				edgeSet[pair<int, Symbol>(itemHash[temp.getStringLALR()], s)] = itemHash[p.getStringLALR()];
				q.push(p);
			}
			else
			{
				int ind=itemHash[p.getStringLALR()];
				itemSet ttemp = vItemSet[ind];
				vItemSet[ind].merge(p);
				edgeSet[pair<int, Symbol>(itemHash[temp.getStringLALR()], s)] = itemHash[vItemSet[ind].getStringLALR()];
				if (!ttemp.deepeq(vItemSet[ind]))
					q.push(vItemSet[ind]);
			}
		}
	}
...
void CalItemSets::ReduceAndConflict()
{
...
int CalItemSets::recur(int state,int*flag)
{
	if (flag[state] == 1)
		return 0;
	flag[state] = 1;
	itemSet is = vItemSet[state];
	set<Symbol> cand;
	vector<item> reduceV;
	...
	for (Symbol s : cand)
	{
		...
		else
		{
			...
			if (rs.Nonassoc.find(s._name) != rs.Nonassoc.end())//非结合性，似乎意思是规约
			{
				//gotoTable[pair<int, Symbol>(state, s)] = next;
			}
			else
